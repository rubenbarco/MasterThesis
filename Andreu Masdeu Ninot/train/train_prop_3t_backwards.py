...
from build_prop_model_backwards import build_model_prop_3t
...
for tasks in [['age', 'gender', 'ethnicity']]:
    ...
    for i in range(3):
        ...
        train_labels = {tasks[0]: np.array(tasks_labels[tasks[0]][0]),
                  ...
                  tasks[2]: np.array(tasks_labels[tasks[2]][0])
                    
                    }

        test_labels = {tasks[0]: np.array(tasks_labels[tasks[0]][1]),
                  tasks[1]: np.array(tasks_labels[tasks[1]][1]),
                  tasks[2]: np.array(tasks_labels[tasks[2]][1])
                    }

        model = build_model_prop_3t(tasks)
        ...
        current_acc = [max(history.history['val_{}_acc'.format(tasks[0])]), 
                       max(history.history['val_{}_acc'.format(tasks[1])]), 
                       max(history.history['val_{}_acc'.format(tasks[2])])]

        if i == 0 or (sum(current_acc) > np.array(acc).sum(axis=1)).all():
            
            model.save('baseline_{}{}{}b.h5'.format(tasks[0], tasks[1], tasks[2]))
        ...
        with open("history{}{}{}{}Baselineb.pkl".format(i, tasks[0], tasks[1], tasks[2]), "wb") as handle:
            pickle.dump(history.history, handle)

    with open("weights_baseline{}{}{}b.pkl".format(tasks[0], tasks[1], tasks[2]), "wb") as handle:
...
for tasks in [['age', 'gender', 'ethnicity']]:
    ...
    weights = pickle.load(open("weights_baseline{}{}{}b.pkl".format(tasks[0], tasks[1], tasks[2]), "rb"))
    ...
    for nb_prop_layers in range(1,6):
        ...
        for i in range(3):
            ...
            model = build_model_prop_3t(tasks, nb_prop_layers)
            ...
            if i == 0 or (sum(current_acc) > np.array(acc).sum(axis=1)).all():
                ...
                model.save('prop{}_{}{}{}b.h5'.format(nb_prop_layers, tasks[0], tasks[1], tasks[2]))
            ...
            with open("history{}{}{}{}{}PropAddback.pkl".format(i, tasks[0], tasks[1], tasks[2], nb_prop_layers), "wb") as handle:
                pickle.dump(history.history, handle)