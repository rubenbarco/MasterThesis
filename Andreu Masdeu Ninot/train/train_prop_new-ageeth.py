...
for tasks in [['age', 'ethnicity']]:
    ...
    for i in range(3):
        ...
        if i == 0 or (sum(current_acc) > np.array(acc).sum(axis=1)).all():
            model.save('baseline_{}{}.h5'.format(tasks[0], tasks[1]))
        ...
        with open("history{}{}{}Baseline.pkl".format(i, tasks[0], tasks[1]), "wb") as handle:
            pickle.dump(history.history, handle)

    with open("weights_baseline{}{}.pkl".format(tasks[0], tasks[1]), "wb") as handle:
...
for tasks in [['age', 'ethnicity']]:
    ...
    weights = pickle.load(open("weights_baseline{}{}.pkl".format(tasks[0], tasks[1]), "rb"))
    ...
    for nb_prop_layers in range(1,6):
        ...
        for i in range(3):
            ...
            if i == 0 or (sum(current_acc) > np.array(acc).sum(axis=1)).all():
                ...
                model.save('prop{}_{}{}.h5'.format(nb_prop_layers, tasks[0], tasks[1]))
            ...
            with open("history{}{}{}{}PropAdd.pkl".format(i, tasks[0], tasks[1], nb_prop_layers), "wb") as handle:
                pickle.dump(history.history, handle)
